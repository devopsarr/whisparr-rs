...
/// struct for typed errors of method [`get_custom_format_schema`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetCustomFormatSchemaError {
    UnknownValue(serde_json::Value),
}
...
pub async fn get_custom_format_schema(configuration: &configuration::Configuration, ) -> Result<(), Error<GetCustomFormatSchemaError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetCustomFormatSchemaError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
