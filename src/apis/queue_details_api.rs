...
/// struct for typed errors of method [`get_queue_details_by_id`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetQueueDetailsByIdError {
...
pub async fn get_queue_details_by_id(configuration: &configuration::Configuration, id: i32) -> Result<models::QueueResource, Error<GetQueueDetailsByIdError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetQueueDetailsByIdError> = serde_json::from_str(&content).ok();
